...
export class Database {
  ...
  backupTo(targetPath: string): void {
    if (!targetPath || targetPath.includes("\0") || /['"]/u.test(targetPath)) {
      throw new Error("SQLite backup path is invalid");
    }
    this.raw.exec(`VACUUM INTO '${targetPath}'`);
  }
  ...
  private applyDataMigrations(): void {
    ...
    if (!applied.has(73)) {
      this.transaction(() => {
        ...
        this.run(
          "INSERT INTO platform_s3_backup_settings (id, include_images, schedule_enabled, schedule_time, retention_count, last_run_status, last_run_error, last_run_json, updated_at) VALUES (1, 1, 0, '03:00', 7, 'idle', '', '{}', ?) ON CONFLICT(id) DO NOTHING",
