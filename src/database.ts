    ...
    if (!applied.has(73)) {
      this.transaction(() => {
        ...
        this.run(`CREATE TABLE IF NOT EXISTS platform_backup_settings (
          ...
          retention_count INTEGER NOT NULL DEFAULT 5 CHECK(retention_count >= 1 AND retention_count <= 100),
          updated_at TEXT NOT NULL
        )`);
        const timestamp = new Date().toISOString();
        this.run(
          "INSERT INTO platform_backup_settings (id, include_images, schedule_hour, schedule_minute, retention_count, updated_at) VALUES (1, 1, 3, 0, 5, ?) ON CONFLICT(id) DO NOTHING",
          timestamp
        );
