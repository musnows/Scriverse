  ...
  private migrate(): void {
    this.raw.exec(`
      ...
      CREATE TABLE IF NOT EXISTS s3_backup_targets (
        ...
        schedule_time TEXT NOT NULL DEFAULT '03:00' CHECK(length(schedule_time) = 5 AND substr(schedule_time, 3, 1) = ':' AND CAST(substr(schedule_time, 1, 2) AS INTEGER) BETWEEN 0 AND 23 AND CAST(substr(schedule_time, 4, 2) AS INTEGER) BETWEEN 0 AND 59),
      ...
      CREATE INDEX IF NOT EXISTS idx_s3_backup_targets_enabled_schedule ON s3_backup_targets(enabled, schedule_time, created_at);
  ...
  private applyDataMigrations(): void {
    ...
    if (!applied.has(73)) {
      this.transaction(() => {
        ...
        this.run("CREATE INDEX IF NOT EXISTS idx_s3_backup_targets_enabled_schedule ON s3_backup_targets(enabled, schedule_time, created_at)");
