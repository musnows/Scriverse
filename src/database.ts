...
export class Database {
    ...
    this.raw.exec(`
      ...
      CREATE TABLE IF NOT EXISTS backup_settings (
        ...
        schedule_time TEXT NOT NULL DEFAULT '03:00' CHECK(
          schedule_time GLOB '[0-2][0-9]:[0-5][0-9]' AND CAST(substr(schedule_time, 1, 2) AS INTEGER) BETWEEN 0 AND 23
        ),
      ...
      CREATE TABLE IF NOT EXISTS backup_targets (
        ...
        connection_status TEXT NOT NULL DEFAULT 'unchecked' CHECK(connection_status IN ('unchecked', 'success', 'failed')),
        last_error TEXT,
      ...
      CREATE TABLE IF NOT EXISTS backup_runs (
        ...
        database_object_key TEXT NOT NULL DEFAULT '',
        database_byte_length INTEGER NOT NULL DEFAULT 0,
      ...
      CREATE INDEX IF NOT EXISTS idx_backup_runs_pending_alert ON backup_runs(acknowledged_at, started_at DESC);
  ...
  private applyDataMigrations(): void {
    ...
    if (!applied.has(73)) {
      this.transaction(() => {
        this.run(
          "INSERT INTO backup_settings (id, schedule_enabled, schedule_time, include_images, retention_count, updated_at) VALUES (1, 0, '03:00', 0, 7, ?) ON CONFLICT(id) DO NOTHING",
  ...
  private recoverInterruptedOperations(): void {
    ...
    this.run(
      `UPDATE backup_runs SET status = 'failed', results_json = ?, finished_at = ?
       WHERE status = 'running'`,
      JSON.stringify([{ status: "failed", error: { message: "服务重启导致备份中断" } }]),
      timestamp
    );
  }
}
