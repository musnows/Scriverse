...
export class Database {
  ...
  createSnapshot(destinationPath: string): void {
    this.raw.prepare("VACUUM INTO ?").run(destinationPath);
  }
  ...
  private applyDataMigrations(): void {
    ...
    if (!applied.has(73)) {
      this.transaction(() => {
        ...
        this.run("CREATE INDEX IF NOT EXISTS idx_s3_backup_targets_enabled ON s3_backup_targets(enabled, updated_at)");
        this.run(
