...
export const AI_CHAT_IMAGE_MAX_BYTES_ENV = "SCRIVERSE_AI_CHAT_IMAGE_MAX_BYTES";

export const DEFAULT_AVATAR_IMAGE_MAX_BYTES = 2 * 1024 * 1024;
export const CHARACTER_AVATAR_IMAGE_MAX_BYTES = 2 * 1024 * 1024;
export const DEFAULT_COVER_IMAGE_MAX_BYTES = 5 * 1024 * 1024;
export const DEFAULT_ATTACHMENT_IMAGE_MAX_BYTES = 30 * 1024 * 1024;
export const MIN_AI_CHAT_IMAGE_MAX_BYTES = 1 * 1024 * 1024;
export const DEFAULT_AI_CHAT_IMAGE_MAX_BYTES = 5 * 1024 * 1024;
...
function resolveUploadLimit(environment: NodeJS.ProcessEnv, name: string, fallback: number, minimum = 1): number {
  const raw = environment[name]?.trim() ?? "";
  if (!/^\d+$/u.test(raw)) return fallback;
  const configured = Number(raw);
  if (!Number.isSafeInteger(configured) || configured < minimum) return fallback;
...
export function resolveImageUploadLimits(environment: NodeJS.ProcessEnv = process.env): ImageUploadLimits {
  return {
    ...
    chatImageBytes: resolveUploadLimit(
      environment,
