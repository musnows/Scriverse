...
const workAiSettingsSchema = z.object({
  ...
  writeTools: z.record(z.string(), z.boolean()).superRefine((value, context) => {
    for (const key of Object.keys(value)) {
      if (!AI_WRITE_TOOL_SWITCH_KEYS.includes(key as typeof AI_WRITE_TOOL_SWITCH_KEYS[number])) {
        context.addIssue({ code: z.ZodIssueCode.custom, path: [key], message: "未知的可写工具开关键" });
      }
    }
  }).optional(),
...
export function createRuntime(options: RuntimeOptions): Runtime {
  ...
  app.post("/api/works/:workId/chat/stream", async (request, response) => {
    ...
    try {
      ...
      // 使用路由内解析出的对话 ID，覆盖首轮消息由服务端新建对话的场景。
      store.discardDraftAiWritePlan(conversationId);
  ...
  app.get("/api/works/:workId/ai-write-plans", (request, response) => {
    ...
    data(response, store.listAiWritePlansPage(request.params.workId, userId, parsePagination(request.query) ?? { page: 1, limit: 30, offset: 0 }, status));
  });
  ...
  app.get("/api/works/:workId/ai-questions", (request, response) => {
    ...
    if (question) {
      const userId = String(request.authUser?.userId ?? "");
      const ownerUserId = store.getAiConversationOwnerUserId(conversationId);
