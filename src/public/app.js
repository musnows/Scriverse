...
async function renderPlatformAiConfig() {
  ...
  host.innerHTML = `<section class="config-section platform-system-prompt-section"><div class="config-section-header"><div><h2>平台全局系统提示词</h2><p>会追加在内置系统提示词之后，并在所有作品的专属提示词之前发送给模型。</p></div></div><div class="field-label"><textarea id="platform-system-prompt" rows="7" aria-label="全局系统提示词" placeholder="例如：默认使用简体中文，避免代替作者做最终决定。">${esc(settings.systemPrompt)}</textarea></div><div class="card-actions"><button id="save-platform-system-prompt" class="primary-button">保存全局提示词</button></div></section><section class="config-section"><div class="config-section-header"><div><h2>多模态读图默认模型</h2><p>Agent 的 image 工具使用这里配置的模型读取设定库图片；作品可以在自己的 AI 设置中覆盖此选择。</p></div></div><div class="config-inline-save"><select id="platform-image-tool-model" aria-label="平台多模态读图默认模型"><option value="">未配置</option>${imageModelOptions}</select><button id="save-platform-image-tool-model" class="ghost-button config-save-button" type="button">保存</button></div></section>${renderProviderCards(providers, models)}`;
...
function openModelDialog(providerId, item = null, provider = null) {
  ...
  const multimodalFields = `<div class="form-field model-multimodal-fields"><label><input id="model-multimodal-enabled" name="multimodalEnabled" type="checkbox" ${values.multimodalEnabled ? "checked" : ""}>支持多模态图片理解</label><label id="model-image-tool-default-field" class="${values.multimodalEnabled && imageDefaultSupported ? "" : "hidden"}"><input id="model-image-tool-default" name="imageToolDefault" type="checkbox" ${values.imageToolDefault ? "checked" : ""}>设为多模态读图工具默认模型</label><small>${imageDefaultSupported ? "启用后可用于设定库图片读取；默认模型只能由 Chat Completions 协议提供。" : "当前供应商协议暂不支持作为多模态读图工具默认模型。"}</small></div>`;
  ...
  const syncMultimodalFields = () => {
    imageDefaultField.hidden = !multimodalInput.checked || !imageDefaultSupported;
