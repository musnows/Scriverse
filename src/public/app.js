...
async function selectChapter(chapterId, { editMode = false } = {}) {
  ...
  if (state.chapter?.id !== chapterId && !(await confirmDiscardChanges("当前章节有未保存修改，仍要切换吗？"))) return false;
  if (selectionGeneration !== chapterSelectionRequestGeneration || state.work?.id !== workId) return false;
  cancelChapterAutoSave();
  ...
  if (!selectedChapter || selectionGeneration !== chapterSelectionRequestGeneration || state.work?.id !== workId) return false;
