...
describe("时间轴界面", () => {
  it("为可编辑时间事件提供经过两次确认的删除操作", async () => {
    ...
    expect(page.text).toContain('/app.js?v=20260814-timeline-delete-toast-v1');
    ...
    expect(application.text.slice(trackTabHandlerStart, trackTabHandlerEnd)).toContain("dismissTimelineDeleteToast();");
