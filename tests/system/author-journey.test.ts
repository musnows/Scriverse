...
describe("作者完整创作流程", () => {
  ...
  it("首屏书架、大纲伏笔、续写守卫和关系银河图资源完整可达", async () => {
    ...
    expect(styles.text).toContain(':is(#users-dialog, #platform-ui-settings-dialog, #s3-backup-dialog, #members-dialog, #appearance-dialog) select:not([multiple]) {');
    ...
    expect(page.text).toContain('id="s3-backup-subdirectory"');
    ...
    expect(application.text).toContain('api("/api/platform/backups")');
    expect(application.text).toContain('api("/api/platform/backups/run"');
    expect(application.text).toContain('formatS3BackupError(error)');
    expect(styles.text).toContain(".s3-backup-card { display: grid;");
