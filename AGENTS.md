...
- 提交新增模块或主要界面前，必须通过内置浏览器在桌面 `1600×900` 和窄屏 `390×844` 下与所选基线模块逐项对照，至少验证标题与操作区对齐、字体和控件尺寸、间距、默认与展开状态、对话框和编辑区域的实际可见尺寸、横向溢出以及控制台错误。截图只能辅助判断，最终报告必须列出参考的既有模块和任何有意保留的差异。
