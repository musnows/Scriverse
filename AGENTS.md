...
- 用户交流时，除 AI 模块外，所有“非正文”模块统一称为“设定库”；“设定库”是非正文模块的统称，不得把它和当前导航中单独名为“设定”的 tab 混为一谈。“设定”只指该 tab，不得用来泛指设定库。
- 当前功能的实现必须最小化，不得在同一改动或同一 commit 中夹带无关重构、顺手修复或行为变化。
- 执行当前任务或必需检查时，如果发现其他模块存在可复现的独立缺陷，且能够以最小、低风险方式修复并独立验证，不得仅因其不属于当前功能而忽略。必须把它视为单独问题：记录复现条件，只修改该缺陷，运行直接相关的测试并创建独立 commit，然后再继续或完成原任务。若修复会显著扩大产品范围、涉及高风险迁移或破坏性操作、改变外部契约，或需要产品决策，则停止扩展修改并向用户报告或请求授权。
...
- “不得混入”约束的是单个 commit 的边界，不代表允许忽略已发现的独立缺陷；符合通用要求中低风险、可独立验证条件的其他模块缺陷，必须另行修复、测试并单独提交。
...
必需测试、类型检查或构建中的任何失败都必须获得明确结论。若失败由其他模块的可复现独立缺陷导致，必须遵循通用要求将其作为单独问题修复、验证并独立提交；禁止以“与本功能无关”“工作区既有状态”或“本功能判定不受影响”为由忽略失败。只有外部环境故障、缺少权限，或修复需要显著扩展范围和用户决策时，才能在完成安全检查后报告为阻塞。
